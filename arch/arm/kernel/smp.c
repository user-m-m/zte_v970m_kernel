...
#ifdef CONFIG_MTK_SCHED_TRACERS
	unsigned long flags;
    struct task_struct *tsk= per_cpu(mtk_next_task, cpu);
    if (unlikely(!sched_stopped)) {
	    local_irq_save(flags);
		trace_int_switch(tsk, 29, 1);
